...
Flask backend serving the hierarchical two-level threat classifier.
Model is downloaded from Hugging Face Hub on first boot.
"""
...
def load_model():
    ...
    if not MODEL_DIR.exists():
        raise RuntimeError(
            f"Model not found. Set HF_REPO env var or place files in ./saved_model/"
        )
...
SEXUAL_EMOJIS         = set("🍆🍑😈💦🔞")
EMOJI_CATEGORY_TOKENS = {
    "aggressive": "[EMOJI_AGGR]",
    "sexual":     "[EMOJI_SEX]",
    "general":    "[EMOJI_GEN]",
}
...
def home():
    return jsonify({"status": "running", "model": "Hierarchical Cyber Threat Classifier v2"})
...
def predict_conversation():
    ...
    full_text = " [SEP] ".join(str(m) for m in messages[-3:])
